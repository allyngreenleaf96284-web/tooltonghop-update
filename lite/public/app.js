...
const LIMITED_MODULES = new Set(["profiles", "full", "interaction", "passwords"]);
...
function setActiveModule(moduleName) {
  if (!LIMITED_MODULES.has(moduleName)) moduleName = "profiles";
...
async function updateLimitedFromOriginal() {
  const button = $("updateLimitedBtn");
  const statusNode = $("limitedUpdateStatus");
  if (button) button.disabled = true;
  if (statusNode) {
    statusNode.textContent = "Đang so sánh với bản gốc...";
    statusNode.className = "config-note";
  }
  try {
    const { data } = await api("/api/limited/update-from-original", { method: "POST", body: "{}" });
    const updated = Number(data?.updated || 0);
    const skipped = Number(data?.skipped || 0);
    const missing = Number(data?.missing || 0);
    const message = updated
      ? `Đã cập nhật ${updated} file, bỏ qua ${skipped}${missing ? `, thiếu ${missing}` : ""}. Hãy tắt mở lại tool để dùng bản mới.`
      : `Không có file nào khác bản gốc. Đã kiểm tra ${skipped + missing} file.`;
    if (statusNode) statusNode.textContent = message;
    setStatus(message, false);
  } catch (error) {
    const message = error.message || "Không cập nhật được từ bản gốc.";
    if (statusNode) {
...
async function updateOnline() {
  ...
  if (button) button.disabled = true;
  if (statusNode) {
    statusNode.textContent = "Đang tải bản cập nhật lite...";
    statusNode.className = "config-note";
  }
  try {
    ...
    const message = updated
      ? `Đã cập nhật ${updated} file lite lên bản ${version}, bỏ qua ${skipped}. Hãy reset backend hoặc tắt mở lại tool.`
      : `Bản giản lược đã ở bản mới nhất ${version}. Không có file cần đổi.`;
    if (statusNode) statusNode.textContent = message;
    setStatus(message, false);
  } catch (error) {
...
if ($("updateLimitedBtn")) $("updateLimitedBtn").addEventListener("click", updateLimitedFromOriginal);
