...
const DEFAULT_UPDATE_MANIFEST_URL = "https://raw.githubusercontent.com/allyngreenleaf96284-web/tooltonghop-update/main/manifest-full.json";
const UPDATE_DOWNLOAD_TIMEOUT_MS = 60000;
const UPDATE_ALLOWED_ROOTS = ["modules/", "public/", "scripts/", "data/"];
const UPDATE_ALLOWED_FILES = new Set(["server.js", "package.json", "README.md", "build-portable.ps1"]);
const UPDATE_BLOCKED_PATTERNS = [
  /^config\.json$/i,
...
const DEFAULT_CONFIG = {
  ...
  knownHideAccounts: {},
  credentialsPath: "",
  credentialSourceSpreadsheetId: "1W9a9PE-0R96dGEjzjfy2BMvI4176emngzsPBOqiu_Qw",
  sellerSpreadsheetId: "",
  ...
  marketplaceCheckTabsPerNick: 5,
  marketplaceCheckTimeoutMs: 90000,
...
async function readConfig() {
  try {
    ...
    loaded.marketplaceCheckTabsPerNick = clampConcurrency(loaded.marketplaceCheckTabsPerNick, DEFAULT_CONFIG.marketplaceCheckTabsPerNick, 20);
    loaded.marketplaceCheckTimeoutMs = Math.max(30000, Math.min(240000, Math.floor(Number(loaded.marketplaceCheckTimeoutMs || DEFAULT_CONFIG.marketplaceCheckTimeoutMs))));
...
async function saveConfig(input) {
  ...
  const next = {
    ...
    credentialSourceSpreadsheetId: String(input.credentialSourceSpreadsheetId || current.credentialSourceSpreadsheetId || DEFAULT_CONFIG.credentialSourceSpreadsheetId).trim(),
    ...
    marketplaceCheckTabsPerNick: clampConcurrency(input.marketplaceCheckTabsPerNick, current.marketplaceCheckTabsPerNick || DEFAULT_CONFIG.marketplaceCheckTabsPerNick, 20),
    marketplaceCheckTimeoutMs: Math.max(30000, Math.min(240000, Math.floor(Number(input.marketplaceCheckTimeoutMs ?? current.marketplaceCheckTimeoutMs ?? DEFAULT_CONFIG.marketplaceCheckTimeoutMs)) || DEFAULT_CONFIG.marketplaceCheckTimeoutMs)),
...
async function getRowsByProfileIds(config, profileIds, options = {}) {
  const { data } = await readManagedSheetData(config, options);
  const wanted = new Set(profileIds.map(normalizeProfileIdText).filter(Boolean));
  const rows = new Map();
  for (const [sheetTitle, sheet] of data) {
    for (const row of sheet.rows) {
      const id = normalizeProfileIdText(row["id hide"]);
...
const marketplaceLinkOrderModule = createMarketplaceLinkOrderTool({
  getHideManager: () => getHmaStudioManager({ fresh: true }),
  dangNhap: dangNhapModule,
  buildToolRow,
  getRowsByProfileIds,
...
function isUpdatePathAllowed(relativePath) {
  ...
  if (UPDATE_BLOCKED_PATTERNS.some((pattern) => pattern.test(normalized))) return false;
  if (UPDATE_ALLOWED_FILES.has(normalized)) return true;
  return UPDATE_ALLOWED_ROOTS.some((root) => normalized.startsWith(root));
}
...
async function runOnlineUpdate(input = {}) {
  ...
  addRuntimeLog(`[update-online] Cap nhat online ${version}: ${updated} file moi, ${skipped} file bo qua`, updated ? "success" : "info", "", {
    tool: "he thong",
...
server.listen(5177, "127.0.0.1", () => {
  console.log("Tool tổng hợp đang chạy tại http://127.0.0.1:5177");
